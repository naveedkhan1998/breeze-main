...
import { createChart, IChartApi, ISeriesApi, ITimeScaleApi, Time, LineData } from "lightweight-charts";
...
const IndicatorChart: React.FC<IndicatorChartProps> = ({ indicators, mode, setTimeScale }) => {
  ...
  useEffect(() => {
    ...
    activeIndicators.forEach((indicator) => {
      if (indicator.name === "RSI") {
        ...
        if (!indicatorChartSeriesRef.current[`${indicator.name}_macd`]) {
          ...
          indicatorChartSeriesRef.current[`${indicator.name}_macd`].setData(indicator.data.map((d) => ({ time: d.time, value: d.macd })));
          indicatorChartSeriesRef.current[`${indicator.name}_signal`].setData(indicator.data.map((d) => ({ time: d.time, value: d.signal })));
          indicatorChartSeriesRef.current[`${indicator.name}_histogram`].setData(indicator.data.map((d) => ({ time: d.time, value: d.histogram })));
        }
      }
    });
    ...
    const inactiveIndicators = indicatorNames.filter((name) => !activeIndicators.some((ind) => ind.name === name));
  ...
  return (
    <>
      {indicators.some((ind) => ind.active && (ind.name === "RSI" || ind.name === "MACD")) && (
        <div ref={indicatorChartContainerRef} className="relative w-full h-full overflow-hidden rounded-lg shadow-lg"></div>
      )}
    </>
  );
};
